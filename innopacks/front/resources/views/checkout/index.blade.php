        ...
        <div class="checkout-info">
          ...
          <div class="checkout-item">
            ...
            <div class="checkout-select-wrap">
              ...
              <div :class="['select-item', current.billing_method_code == 'vnpay' ? 'active' : '']"
                @click="updateCheckout('billing_method_code', 'vnpay')">
                <div class="left">
                  <i class="bi bi-circle"></i>
                  <div class="select-title">Thanh toán VNPay</div>
                </div>
                <div class="icon">
...
@push('footer')
  <script>
    ...
    const api = {
      ...
      vnpayPayment: @json(front_route('payment.vnpay')), 
    }
...
const submitCheckout = () => {
  ...
  axios.post(api.checkoutConfirm, current).then(function(res) {
    if (res.success) {
      ...
      if (current.billing_method_code === 'vnpay') {
        // Sử dụng URL từ api object
        axios.post(api.vnpayPayment, {
          order_number: res.data.number,
          total_vnpay: res.data.total
        }).then(function(vnpayRes) {
          if (vnpayRes.success) {
            window.location.href = vnpayRes.data.redirect_url;
          }
        }).catch(function(error) {
          console.error('VNPay Error:', error);
          layer.msg('Có lỗi xảy ra khi thanh toán VNPay');
        });
      } else {
