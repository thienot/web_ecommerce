...
// func show origin price and code currency to put data to payment (vnpay)
if (! function_exists('currency_payment_data')) {
    /**
     * Get payment-ready currency data (số thuần + currency code)
     * 
     * @param  $price
     * @param  string  $currency
     * @return array
     */
    function currency_payment_data($price, string $currency = ''): array
    {
        if (! $currency) {
            $currency = is_admin() ? system_setting('currency') : current_currency_code();
        }

        return Currency::getInstance()->getPaymentAmount($price, $currency);
    }
}
