...
class Currency
{
    ...
    /**
     * Get payment-ready amount data
     * 
     * @param $price
     * @param $currency
     * @return array
     */
    public function getPaymentAmount($price, $currency): array
    {
        $currency = strtolower($currency);
        $currencyRow = $this->currencies[strtoupper($currency)] ?? null;
        ...
        $price = (float) $price;
        $rate = (float) $currencyRow->value;
        $decimal = (int) $currencyRow->decimal_place;
        
        // Tính conversion (chỗ tính ra 2,500,000)
        $convertedPrice = $rate ? ($price * $rate) : $price;
        
        // Round theo decimal của currency  
        $finalAmount = round($convertedPrice, $decimal);
        
        return [
            'amount' => $finalAmount,                    // 2500000 (VND) hoặc 100 (USD)
            'currency_code' => strtoupper($currency),    // 'VND' hoặc 'USD'
            'decimal_places' => $decimal,                // 0 (VND) hoặc 2 (USD)
            'original_amount' => $price,                 // 100 (USD gốc)
            'exchange_rate' => $rate,                    // 25000 (VND rate)
            'formatted' => $this->format($price, $currency, 0, true) // "2,500,000₫"
        ];
    }
