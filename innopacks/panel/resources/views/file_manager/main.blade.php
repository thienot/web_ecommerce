...
@section('content')
  <div id="app">
          ...
          <el-col :span="12">
            <el-button-group>
              ...
              <el-button size="small" @click="createFolder">
                <i class="el-icon-folder-add"></i> Tạo thư mục
              </el-button>
              ...
              <el-button size="small" :type="isMultiSelectMode ? 'primary' : 'default'" @click="toggleMultiSelectMode">
                <i class="el-icon-check"></i> Chọn nhiều
              </el-button>
              <el-button v-if="isMultiSelectMode" size="small" @click="selectAll">
                <i class="el-icon-finished"></i> Chọn tất cả
              </el-button>

              <el-button type="primary" size="small"@click="handleConfirm">Chọn và gửi</el-button>
          ...
          <el-col :span="12" style="text-align: right">
            <el-button-group>
              <el-button size="small" :disabled="selectedFiles.length !== 1" @click="renameSelectedFile">
                <i class="el-icon-edit"></i> Đổi tên
              </el-button>
              ...
              <el-button size="small" :disabled="!selectedFiles.length" @click="moveFiles">
                <i class="el-icon-folder"></i> Di chuyển đến
              </el-button>
              <el-button size="small" :disabled="!selectedFiles.length" @click="copyFiles">
                <i class="el-icon-document-copy"></i> Sao chép
              </el-button>
            </el-button-group>
          </el-col>
        </el-row>
                  ...
                  <el-col :xs="12" :sm="8" :md="6" :lg="4"
                    ...
                    <div :class="['file-card', selectedFiles.includes(file.id || file.path) ? 'selected' : '']"
                      ...
                      <div class="file-info">
                        ...
                        <p class="file-type">@{{ file.is_dir ? 'Thư mục' : file.mime }}</p>
                      </div>
                    </div>
                  </el-col>
                </el-row>
    ...
    <!-- 新建文件夹对话框 -->
    <el-dialog title="Tạo thư mục mới" :visible.sync="folderDialog.visible" width="400px">
      <el-form :model="folderDialog.form" label-width="80px">
        <el-form-item label="Tên thư mục">
          <el-input v-model="folderDialog.form.name" placeholder="Vui lòng nhập tên thư mục"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer">
        <el-button @click="folderDialog.visible = false">Hủy</el-button>
        <el-button type="primary" @click="submitCreateFolder">Xác nhận</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog title="Upload file" :visible.sync="uploadDialog.visible" width="500px">
      <el-upload class="file-uploader" drag multiple :action="uploadUrl" :headers="uploadHeaders"
        ...
        <div class="el-upload__text"> Kéo tệp vào đây hoặc <em>nhấn để tải lên</em></div>
        <div class="el-upload__tip" slot="tip">Hỗ trợ các tệp hình ảnh định dạng JPG, JPEG, PNG, GIF</div>
      </el-upload>
    </el-dialog>
    ...
    <el-dialog title="重命名" :visible.sync="renameDialog.visible" custom-class="rename-dialog" width="500px">
      ...
      <span slot="footer">
        <el-button @click="renameDialog.visible = false">Hủy</el-button>
        <el-button type="primary" @click="submitRename">Xác nhận</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog title="移动到" :visible.sync="moveDialog.visible" width="400px">
      ...
      <span slot="footer">
        <el-button @click="moveDialog.visible = false">Hủy</el-button>
        <el-button type="primary" @click="submitMove">Xác nhận</el-button>
      </span>
    </el-dialog>
    ...
    <div class="file-card-context-menu" v-show="contextMenu.visible" :style="contextMenu.style">
      <ul>
        <li @click="renameFile"><i class="el-icon-edit"></i> Đổi tên</li>
        <li @click="deleteFile"><i class="el-icon-delete"></i> Xóa</li>
        <li @click="moveFile"><i class="el-icon-folder"></i> Di chuyển</li>
        <li @click="copyFile"><i class="el-icon-document-copy"></i> Sao chép</li>
      </ul>
    </div>
    ...
    <el-dialog title="复制到" :visible.sync="copyDialog.visible" width="400px">
      ...
      <span slot="footer">
        ...
        <el-button type="primary" @click="submitCopy">Xác nhận</el-button>
      </span>
    </el-dialog>
    ...
    <div v-if="folderContextMenu.visible" class="file-card-context-menu"
      ...
      <ul>
        ...
        <li @click="moveFolder">
          <i class="el-icon-position"></i> Di chuyển
        </li>
    ...
    <el-dialog title="重命名文件夹" :visible.sync="folderRenameDialog.visible" width="400px">
      <el-form :model="folderRenameDialog.form" label-width="80px">
        <el-form-item label="文件夹名">
          <el-input v-model="folderRenameDialog.form.newName" placeholder="Nhập tên thư mục"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer">
        <el-button @click="folderRenameDialog.visible = false">Hủy</el-button>
        <el-button type="primary" @click="submitFolderRename">Xác nhận</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog title="移动文件夹" :visible.sync="folderMoveDialog.visible" width="400px">
      ...
      <span slot="footer">
        <el-button @click="folderMoveDialog.visible = false">Hủy</el-button>
        <el-button type="primary" @click="submitFolderMove">Xác nhận</el-button>
      </span>
    </el-dialog>
      ...
      methods: {
        ...
        submitCreateFolder() {
          if (!this.folderDialog.form.name) {
            this.$message.warning('Vui lòng nhập!');
            return;
          }
          ...
          http.post('file_manager/directories', {
            ...
            if (res.success) {
              ...
              this.$message.error(res.message || 'Tạo thất bại!');
            }
          }).catch(err => {
            this.$message.error('Tạo không thành công: ' + err.message);
          });
        },
        deleteFiles() {
          if (!this.selectedFiles.length) return;

          this.$confirm('Bạn có chắc chắn muốn xóa các tệp đã chọn?', '提示', {
        ...
        beforeUpload(file) {
          ...
          if (!isLt2M) {
            this.$message.error('Kích thước tệp không được vượt quá 8MB！');
            return false;
          }
          if (isVideo || isDoc) {
            ...
            http.post('file_manager/upload', formData)
              .then(res => {
                if (res.success) {
                  ...
                  this.$message.error(res.message || 'Upload thất bại!');
                }
              })
              .catch(err => {
                this.$message.error('Upload lỗi: ' + err.message);
              });
            return false;
          } else {
            this.cropImage(file);
        ...
        cropImage(file) {
          ...
          <div class="cropper-controls">
            <button class="el-button el-button--default el-button--small cancel-btn">Hủy</button>
            <button class="el-button el-button--primary el-button--small confirm-btn">Xác nhận</button>
          </div>
        `;
        ...
        handleUploadSuccess(response, file, fileList) {
          if (response.success) {
            ...
            this.$message.error(response.message || 'Tải lên lỗi!');
          }
        ...
        handleUploadError(err, file) {
          this.$message.error('Tải lên thất bại: ' + (err.message || 'Lỗi không xác định'));
        },
        ...
        deleteFile() {
          const file = this.contextMenu.file;
          this.$confirm('Xác nhận xóa tệp này?', '提示', {
        ...
        submitMove() {
          if (!this.moveDialog.targetPath) {
            this.$message.warning('Vui lòng chọn thư mục đích!');
            return;
          }
        ...
        submitFolderRename() {
          if (!this.folderRenameDialog.form.newName) {
            this.$message.warning('Vui lòng nhập tên mới!');
            return;
          }
        ...
        deleteFolder() {
          const folder = this.folderContextMenu.folder;
          this.$confirm('Xác nhận xóa thư mục này?', '提示', {
            confirmButtonText: 'Xác nhận',
            cancelButtonText: 'Hủy',
            type: 'warning'
          }).then(() => {
        ...
        handleFolderMoveTargetSelect(data) {
          ...
          if (data.path === this.folderMoveDialog.folder.path ||
            ...
            this.$message.warning('Không thể di chuyển vào chính nó hoặc thư mục con của nó!');
            return;
          }
        ...
        handleNodeDrop(draggingNode, dropNode, type) {
          ...
          http.post('file_manager/move_directories', {
            ...
            this.loadFolders();
            this.$message.error(err.message || 'Lỗi!');
          });
        },
        ...
        confirmSelection() {
          if (this.isIframeMode && window.parent.fileManagerCallback) {
            if (this.selectedFiles.length === 0) {
              this.$message.warning('Vui lòng chọn ít nhất một tệp!');
              return;
            }
